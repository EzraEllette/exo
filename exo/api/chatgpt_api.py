...
class ChatGPTAPI:
    def __init__(
        ...
        on_chat_completion_request: Callable[
            [str, ChatCompletionRequest, str], None
        ] = None,
    ):
        self.node = node
        ...
        cors.add(
            self.app.router.add_post(
                "/chat/token/encode", self.handle_post_chat_token_encode
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post(
                "/v1/chat/token/encode", self.handle_post_chat_token_encode
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post(
                "/chat/completions", self.handle_post_chat_completions
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post(
                "/v1/chat/completions", self.handle_post_chat_completions
            ),
            {"*": cors_options},
        )
    ...
    async def handle_get_models(self, request):
        return web.json_response(
            [
                ...
                for model_name, _ in model_base_shards.items()
            ]
        )

    async def handle_post_chat_token_encode(self, request):
        data = await request.json()
        shard = model_base_shards.get(data.get("model", "llama-3.1-8b"), {}).get(
    ...
    async def handle_post_chat_completions(self, request):
        ...
        tokenizer = await resolve_tokenizer(shard.model_id)
        if DEBUG >= 4:
            print(f"Resolved tokenizer: {tokenizer}")
        ...
        try:
            if DEBUG >= 2:
                print(
                    f"Waiting for response to finish. timeout={self.response_timeout}s"
                )
            ...
            if stream:
                ...
                async def stream_result(
                    request_id: str, tokens: List[int], is_finished: bool
                ):
                    prev_last_tokens_len = self.prev_token_lens.get(request_id, 0)
                    self.prev_token_lens[request_id] = max(
                        prev_last_tokens_len, len(tokens)
                    )
                    ...
                    eos_token_id = (
                        ...
                        and isinstance(tokenizer._tokenizer, AutoTokenizer)
                ...
                def on_result(_request_id: str, tokens: List[int], is_finished: bool):
                    self.stream_tasks[request_id] = asyncio.create_task(
                        stream_result(request_id, tokens, is_finished)
                    )

                    return _request_id == request_id and is_finished

                _, tokens, _ = await callback.wait(
                    on_result, timeout=self.response_timeout
                )
                if (
                    ...
                    if DEBUG >= 2:
                        print(
                            "Pending stream task. Waiting for stream task to complete."
                        )
                    try:
                        await asyncio.wait_for(
                            self.stream_tasks[request_id], timeout=30
                        )
                    except asyncio.TimeoutError:
                ...
                eos_token_id = (
                    tokenizer.special_tokens_map.get("eos_token_id")
                    if isinstance(getattr(tokenizer, "_tokenizer", None), AutoTokenizer)
                    else tokenizer.eos_token_id
                )
                if DEBUG >= 2:
                    print(
                        f"Checking if end of tokens result {tokens[-1]=} is {eos_token_id=}"
                    )
            ...
            return web.json_response(
                {"detail": "Response generation timed out"}, status=408
            )
        finally:

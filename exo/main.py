...
parser.add_argument(
    "--listen-port", type=int, default=5678, help="Listening port for discovery"
)
parser.add_argument(
...
parser.add_argument(
    "--disable-tui", action=argparse.BooleanOptionalAction, help="Disable TUI"
)
...
chatgpt_api_endpoints = [
    f"http://{ip}:{args.chatgpt_api_port}/v1/chat/completions"
    for ip in get_all_ip_addresses()
]
web_chat_urls = [
    f"http://{ip}:{args.chatgpt_api_port}" for ip in get_all_ip_addresses()
]
if DEBUG >= 0:
...
topology_viz = (
    TopologyViz(
        chatgpt_api_endpoints=chatgpt_api_endpoints, web_chat_urls=web_chat_urls
    )
...
api = ChatGPTAPI(
    ...
    on_chat_completion_request=lambda req_id, __, prompt: (
        topology_viz.update_prompt(req_id, prompt) if topology_viz else None
    ),
)
node.on_token.register("update_topology_viz").on_next(
    lambda req_id, tokens, __: (
        topology_viz.update_prompt_output(
...
async def run_model_cli(
    node: Node, inference_engine: InferenceEngine, model_name: str, prompt: str
):
    shard = model_base_shards.get(model_name, {}).get(
        inference_engine.__class__.__name__
    )
    if not shard:
        print(

...
class StandardNode(Node):
        ...
        try:
            ...
            if status_data.get("type", "") == "node_status":
                if status_data.get("status", "").startswith("start_"):
                    ...
                    if (
                        status_data.get("node_id")
                        == self.current_topology.active_node_id
                    ):
            ...
            if status_data.get("type", "") == "download_progress":
                if DEBUG >= 8:
                    print(
                        f"Download progress from {status_data.get('node_id')}: {status_data.get('progress')}"
                    )
                download_progress = RepoProgressEvent.from_dict(
    ...
    async def process_prompt(
        ...
        resp = await self._process_prompt(
            base_shard, prompt, image_str, request_id, inference_state
        )
    ...
    async def _process_prompt(
        ...
        if DEBUG >= 2:
            print(
                f"[{request_id}] process prompt: {base_shard=} {shard=} {prompt=} {image_str=}"
            )
        if shard.start_layer != 0:
            if DEBUG >= 2:
                print(
                    f"[{request_id}] forwarding to next shard: {base_shard=} {shard=} {prompt=} {image_str=}"
                )
        ...
        is_finished = (
            is_finished
            or len(self.buffered_token_output[request_id][0])
        ...
        asyncio.create_task(
            self.broadcast_result(
                request_id, self.buffered_token_output[request_id][0], is_finished
            )
        )  # TODO: this is n^2 communication complexity

        if result.size == 1:
            self.buffered_token_output[request_id][0].append(result.item())
            self.trigger_on_token_callbacks(
                request_id, self.buffered_token_output[request_id][0], is_finished
            )

        if DEBUG >= 2:
            print(
                f"[{request_id}] result size: {result.size}, is finished: {is_finished}, buffered tokens: {len(self.buffered_token_output[request_id][0])}"
            )

        if not is_finished:
            asyncio.create_task(
                self.forward_to_next_shard(
                    shard,
                    result,
                    request_id,
                    image_str=image_str,
    ...
    async def _process_tensor(
        ...
        try:
            ...
            result, inference_state, is_finished = (
            ...
            if not is_finished:
                asyncio.create_task(
                    self.forward_to_next_shard(
                        shard, result, request_id, inference_state=inference_state
                    )
                )
    ...
    async def forward_to_next_shard(
        ...
        current_partition_index = next(
            (i for i, p in enumerate(partitions) if p.node_id == self.id), None
        )
        if DEBUG >= 1:
        ...
        if current_partition_index is not None:
            ...
            if DEBUG >= 2:
                print(
                    f"Computed next from: {shard}, {self.topology}. Next partition: {next_partition}"
                )
            ...
            target_peer = next(
                (p for p in self.peers if p.id() == next_partition.node_id), None
            )
            if not target_peer:
            ...
            if DEBUG >= 1:
                print(
                    f"Sending tensor_or_prompt to {target_peer.id()}: {tensor_or_prompt}"
                )
    ...
    async def update_peers(self, wait_for_peers: int = 0) -> bool:
        ...
        peers_updated = [
            ...
            if peer.id() in current_peer_ids
            and any(p.addr() != peer.addr() for p in self.peers if p.id() == peer.id())
        ]
        peers_unchanged = [
            peer
            for peer in next_peers
            if peer.id() in current_peer_ids
            and all(p.addr() == peer.addr() for p in self.peers if p.id() == peer.id())
        ]
        peers_to_disconnect = [
            peer for peer in peers_removed if await peer.is_connected()
        ]
        peers_to_connect = [
            peer
            for peer in peers_added + peers_updated + peers_unchanged
        ...
        successful_disconnects = [
            peer
            for peer, result in zip(peers_to_disconnect, disconnect_results)
            if result is True
        ]
        failed_disconnects = [
            peer
        ...
        successful_connects = [
            peer
            for peer, result in zip(peers_to_connect, connect_results)
            if result is True
        ]
        failed_connects = [
            peer
        ...
        if DEBUG >= 1:
            if successful_disconnects:
                print(
                    f"Successfully disconnected peers: {_pretty(successful_disconnects)}"
                )
    ...
    async def collect_topology(
        self, visited: set[str] = set(), max_depth: int = 4
    ) -> Topology:
        ...
        next_topology.active_node_id = (
            self.topology.active_node_id
    ...
    def trigger_on_token_callbacks(
        self, request_id: str, tokens: List[int], is_finished: bool
    ) -> None:
        if DEBUG >= 2:
            print(
                f"Triggering all on_token callbacks with {request_id=} num_tokens={len(tokens)} {is_finished=}"
            )
    ...
    async def broadcast_result(
        ...
        async def send_result_to_peer(peer):
            try:
                await asyncio.wait_for(
                    peer.send_result(request_id, result, is_finished), timeout=15.0
                )
        ...
        await asyncio.gather(
            *[send_result_to_peer(peer) for peer in self.peers], return_exceptions=True
        )
    ...
    async def broadcast_opaque_status(self, request_id: str, status: str) -> None:
        ...
        async def send_status_to_peer(peer):
            try:
                await asyncio.wait_for(
                    peer.send_opaque_status(request_id, status), timeout=15.0
                )
            except asyncio.TimeoutError:
        ...
        await asyncio.gather(
            *[send_status_to_peer(peer) for peer in self.peers], return_exceptions=True
        )
